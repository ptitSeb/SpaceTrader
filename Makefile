...
SYSDIR=$(MOUNT_DIR)/unix
SQLDIR=$(MOUNT_DIR)/sql
GLEWDIR=$(LIBS_DIR)/glew/src
GDIR=$(MOUNT_DIR)/game
...
ZDIR=$(MOUNT_DIR)/zlib
Q3ASMDIR=$(MOUNT_DIR)/../tools/qvm/asm
LBURGDIR=$(MOUNT_DIR)/../tools/qvm/lcc/lburg
Q3CPPDIR=$(MOUNT_DIR)/../tools/qvm/lcc/cpp
Q3LCCETCDIR=$(MOUNT_DIR)/../tools/qvm/lcc/etc
Q3LCCSRCDIR=$(MOUNT_DIR)/../tools/qvm/lcc/src
...
ifeq ($(PLATFORM),pandora)
  ...
  BASE_CFLAGS = -Wall -fno-strict-aliasing -Wimplicit \
    ...
    -ftree-vectorize -fsingle-precision-constant -DTR_COMPILE_NO_SSE -g
  CLIENT_CFLAGS = $(SDL_CFLAGS)
  SERVER_CFLAGS =
  USE_LOCAL_HEADERS = 
  USE_OPENAL_DLOPEN=0
...
#Sago: Here I get vorbis to compile in Windows:
  ...
  BASE_CFLAGS += -DPANDORA -DARM -DNEON
...
TOOLS_CFLAGS += $(TOOLS_OPTIMIZE) \
                ...
                -I$(Q3LCCSRCDIR) \
                -I$(LBURGDIR) -I$(MOUNT_DIR) -fsigned-char -g
TOOLS_LIBS =
TOOLS_LDFLAGS =
...
define DO_GAME_Q3LCC
$(echo_cmd) "GAME_Q3LCC $<"
$(Q)$(Q3LCC) $(STANDALONE_CFLAGS) -DGAME -o $@ $<
endef
...
Q3OBJ = \
  ...
  \
  $(B)/client/sql.o \
  $(B)/client/sql_alter.o \
  $(B)/client/sql_create.o \
  $(B)/client/sql_delete.o \
  $(B)/client/sql_eval.o \
  $(B)/client/sql_format.o \
  $(B)/client/sql_insert.o \
  $(B)/client/sql_msg.o \
  $(B)/client/sql_parse.o \
  $(B)/client/sql_parser.o \
  $(B)/client/sql_search.o \
  $(B)/client/sql_select.o \
  $(B)/client/sql_strings.o \
  $(B)/client/sql_update.o \
  \
  ...
  $(B)/client/l_struct.o \
  $(B)/client/tr_x42.o \
  $(B)/client/tr_x42-animverts.o \
  $(B)/client/tr_x42-load.o \
  $(B)/client/tr_x42-math.o \
  $(B)/client/tr_x42-pose.o \
  $(B)/client/tr_x42-sse.o \
  $(B)/client/tr_backend.o \
  $(B)/client/tr_bsp.o \
  $(B)/client/tr_cmds.o \
  $(B)/client/tr_curve.o \
  $(B)/client/tr_draw2d.o \
  ...
  $(B)/client/tr_init.o \
  $(B)/client/tr_light.o \
  $(B)/client/tr_lightmapconv.o \
  $(B)/client/tr_main.o \
  $(B)/client/tr_marks.o \
  $(B)/client/tr_mesh.o \
  $(B)/client/tr_model.o \
  $(B)/client/tr_noise.o \
  $(B)/client/tr_postproc.o \
  $(B)/client/tr_scene.o \
  $(B)/client/tr_screenshot.o \
  $(B)/client/tr_shade.o \
  $(B)/client/tr_shade_calc.o \
  $(B)/client/tr_shade_programs.o \
  $(B)/client/tr_shader.o \
  $(B)/client/tr_shadows.o \
  $(B)/client/tr_sky.o \
  $(B)/client/tr_state.o \
  $(B)/client/tr_state-states.o \
  $(B)/client/tr_state-states-textures.o \
...
ifeq ($(PLATFORM),mingw32)
  ...
  LIBS += \
    -lX11 -lGL -lGLEW -lpng16 -lXxf86vm
endif
...
Q3CGOBJ_ = $(SDK_Q3CGOBJ) $(B)/$(SDK_GAMENAME)/game/bg_lib.o 
...
Q3GOBJ_ = $(SDK_Q3GOBJ) $(B)/$(SDK_GAMENAME)/game/bg_lib.o
Q3GOBJ = $(Q3GOBJ_) $(B)/$(SDK_GAMENAME)/game/g_syscalls.o
endif
Q3GVMOBJ = $(Q3GOBJ_:%.o=%.asm)

ifndef SDK_GAMENAME
$(B)/baseq3/game$(SHLIBNAME): $(Q3GOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CFLAGS) $(SHLIBLDFLAGS) -o $@ $(Q3GOBJ)

$(B)/baseq3/vm/game.qvm: $(Q3GVMOBJ) $(GDIR)/g_syscalls.asm $(Q3ASM)
	$(echo_cmd) "Q3ASM $@"
	$(Q)$(Q3ASM) $(ASMFLAGS) -o $@ $(Q3GVMOBJ) $(GDIR)/g_syscalls.asm
else
$(B)/$(SDK_GAMENAME)/game$(SHLIBNAME): $(Q3GOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CFLAGS) $(SHLIBLDFLAGS) -o $@ $(Q3GOBJ)

$(B)/$(SDK_GAMENAME)/vm/game.qvm: $(Q3GVMOBJ) $(GDIR)/g_syscalls.asm $(Q3ASM)
	$(echo_cmd) "Q3ASM $@"
	$(Q)$(Q3ASM) $(ASMFLAGS) -o $@ $(Q3GVMOBJ) $(GDIR)/g_syscalls.asm
endif
...
ifndef SDK_GAMENAME
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(B)/baseq3/cgame$(SHLIBNAME) \
					$(COPYDIR)/baseq3/.
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(B)/baseq3/game$(SHLIBNAME) \
					$(COPYDIR)/baseq3/.
	...
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(B)/$(SDK_GAMENAME)/game$(SHLIBNAME) \
					$(COPYDIR)/$(SDK_GAMENAME)/.
